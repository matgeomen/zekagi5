...
export default function App(): JSX.Element {
  ...
  const handleSendMessage = async (messageText: string) => {
    ...
    try {
      // İnternet öğrenme sistemi aktifse ve cevap bulunamazsa otomatik arama yap
      let shouldSearchInternet = false;
      let internetResponse = null;

      if (isInternetEnabled) {
        // Önce normal cevap denetle
        const normalResult = await processUserInput(messageText);
        
        if (!normalResult.response || normalResult.confidence < 0.6 || 
            normalResult.response.includes('bilmiyorum') || 
            normalResult.response.includes('bulunamadı') ||
            normalResult.response === 'MODAL_TRIGGER_UNKNOWN' ||
            normalResult.response.length < 10) { // Çok kısa cevaplar da internet araması yapsın
      ...
      if (shouldSearchInternet) {
        ...
        try {
          console.log(`🌐 İnternet araması başlatılıyor: "${messageText}"`);
          const learningData = await internetLearning.searchAndLearn(messageText);
          
          if (learningData && learningData.synthesis && learningData.synthesis.length > 15) {
            // Sadece anlamlı ve yeterli uzunlukta cevapları kabul et
            const trainingPair: TrainingPair = {
              input: messageText,
              output: learningData.synthesis
            };
            ...
            internetResponse = learningData.synthesis;
            console.log(`✅ İnternet öğrenme başarılı: ${learningData.concepts.length} kavram`);
            addToast(`🎓 İnternetten yeni bilgi öğrenildi!`, 'success');
          } else {
            console.log('❌ İnternet aramasından alakalı sonuç alınamadı');
            addToast('⚠️ Bu konuda güncel bilgi bulunamadı', 'warning');
          }
        } catch (error) {
      ...
      // Process with neural network (eğer internet araması yapılmamışsa)
      let result;
      if (!internetResponse) {
        result = await processUserInput(messageText);
      } else {
        // İnternet cevabı varsa mock result oluştur
        result = {
